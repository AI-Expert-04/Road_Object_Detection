...
def visualize_detections(image, boxes, classes, scores):
    for box, _cls, score in zip(boxes, classes, scores):
        ...
        cv2.putText(image, text, (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2, cv2.LINE_AA)
    return image
...
class RetinaNetBody(tf.keras.Model):
    def __init__(self, num_classes, backbone=None, **kwargs):
        super(RetinaNetBody, self).__init__(name="RetinaNet", **kwargs)
...
class RetinaNet:
    def __init__(self):
        ...
        f = open('../models/retinanet/classes.txt')
        self.class_names = [str(i).strip() for i in f.readlines()]
        f.close()

    def build(self):
        resnet50_backbone = get_backbone()
        self.model = RetinaNetBody(self.num_classes, resnet50_backbone)

    def load(self):
        weights_dir = "../models/retinanet"
        ...
        self.inference_model = tf.keras.Model(inputs=image, outputs=detections)

    def predict(self, image):
        input_image = tf.cast(image, dtype=tf.float32)
        input_image, ratio = prepare_image(input_image)
        detections = self.inference_model.predict(input_image)
